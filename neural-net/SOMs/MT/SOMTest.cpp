...
// 2 experimenter instructions: Grasp or Categorise
int inputSize=2;

int visionInput[2]; 

int VisionInput0[2]= {0,1};
int VisionInput1[2]= {1,0};
...
//	input -> setFunction(RampFunction(-100, 100, -100, +100));
	...
	for (int e = 1; e <= numIteration; e++) { //numIteration; e++){	
		...
		int line  = Random::flatInt(0,2);
		
		if (line == 0) {memcpy(visionInput, VisionInput0, sizeof(int)*2);} 

		if (line == 1) {memcpy(visionInput, VisionInput1, sizeof(int)*2);}
	...
	//testSOM();
	//nnfw::saveXML("results/MT_SOM.xml",net);    
}
	return 0;	   
}
...
void testSOM() {
	...
	// Clear inputs and outputs of the net
	...
	for (int test = 0; test < 2; test++) {
		...
		if (task == 0) {memcpy(visionInput, VisionInput0, sizeof(int)*2);} 

		if (task == 1) {memcpy(visionInput, VisionInput1, sizeof(int)*2);}
